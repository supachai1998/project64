    ...
    return (
        <div className="flex flex-col w-full px-4 h-96 sm:h-1/4 sm:gap-y-2 gap-y-12  text-center">
            <div className="h-1/4">
                ...
                <p className="-mt-2  text-gray-500 text-3xl" ><span className="mr-10">{headerData.at(-1).content}</span></p>
            </div>
            {headerData.slice(0, headerData.length - 1).map(val => <>
                <hr className="w-1/12 mx-auto border-red-600" />
                <div className="h-1/4 mx-auto">
                    
                    <p className="pr-1 text-2xl font-thin  lg:text-2xl font-Charm">{val.title}</p>
                    <Typography className={`pr-2 -mt-2 overflow-auto zm:h-20 text-left  ${val.title === "วิธีการทำ" ? "max-w-2xl" : ""}`}><span className='text'>{"val.content\nsadl';dsa\nasdlasdkas;ka;d"}</span></Typography>
                    {/* <p className="pr-2 -mt-2 overflow-auto zm:h-20 text-left max-w-2xl" >{val.content}</p> */}
                </div>
            </>)}
